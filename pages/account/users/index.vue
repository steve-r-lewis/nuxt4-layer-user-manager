  ...
  <div>
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      ...
      <div class="flex gap-3">
        <button class="btn bg-fill-tertiary-default text-pen-base-default border border-edge-base-default hover:bg-fill-tertiary-hover">
          ...
          <tr v-for="u in users" :key="u.account.id" class="hover:bg-fill-base-hover transition group">
            ...
            <td class="px-6 py-4">
              <div class="flex flex-wrap gap-2">
                  <span
                    v-for="tenantId in u.account.tenantIds.slice(0, 2)"
                    :key="tenantId"
                    class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-fill-tertiary-default text-pen-base-default border border-edge-base-default"
                  >
                    {{ tenantId.substring(0, 4) }}...
                  </span>
                <span v-if="u.account.tenantIds.length > 2" class="text-xs text-pen-muted-default self-center">
                    +{{ u.account.tenantIds.length - 2 }}
                  </span>
              </div>
            </td>
            ...
            <td class="px-6 py-4">
                <span
                  ...
                  :class="u.account.status === 'active' ? 'bg-fill-success-default text-pen-success-default' : 'bg-fill-tertiary-default text-pen-muted-default'"
                >
